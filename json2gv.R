###This program converts a slightly-modified JSON file obtained from sparql (a Wikipedia database query tool) to a .gv file, with formatting obtained from page 17 of http://www.graphviz.org/pdf/dotguide.pdf
...
for(i in 1:ncol(weight_size)){
  weight_char_string <- paste(weight_char_string, "node [fontsize=", 32*as.numeric(names(weight_size)[i]), "];\n", sep="")
